...
from glimpse_extractor_bcauto import densenet121_racnn_glimpse_extractor 
...
def densenet121_racnn_gl_with_orig(weights=None, base_model_weights=None,
        num_classes=200, glimpse_size=35, use_gpu=True, skip_con=False):
    
    model = DenseNet_RACNN_GL_With_Orig(base_model_weights=base_model_weights,
                num_classes=num_classes, use_gpu=use_gpu,
                glimpse_size=glimpse_size, skip_con=skip_con, conv2_only=True)
...
def densenet121_racnn_gl(weights=None, num_classes=200, glimpse_only=False,
        ...
        glimpse_size=0, skip_con=False, conv2_only=False):
    if weights is not None:
        base_pretrained = False
    else:
        base_pretrained = True
    model = DenseNet_RACNN_GL(num_classes=200, base_pretrained=base_pretrained,
            use_gpu=use_gpu, glimpse_only=glimpse_only,
            glimpse_size=glimpse_size, skip_con=skip_con, conv2_only=conv2_only)
...
class DenseNet_RACNN_GL_With_Orig(nn.Module):
    def __init__(self, base_model_weights=None, num_classes=200,
            use_gpu=True, glimpse_size=0, skip_con=False, glimpses=2,
            conv2_only=True):
        super(DenseNet_RACNN_GL_With_Orig, self).__init__()
        
        self.num_fltrs = 1024 # num filters in output of conv
        self.base_model = densenet121_racnn_gl(weights=base_model_weights,
                skip_con=skip_con, use_gpu=use_gpu, conv2_only=conv2_only)
        self.g = glimpses 
        del self.base_model.fc1
        del self.base_model.fc2
        self.fc1 = nn.Linear(self.num_fltrs * (glimpses+1), self.num_fltrs/2)
        self.fc2 = nn.Linear(self.num_fltrs/2, num_classes)
    ...
    def forward(self, x):
        ...
        glimpses, m, m_raw = self.base_model.glimpse(x)
        orig = x.view(s, 1, c, H, W)

        f = torch.cat([glimpses, orig], dim=1)
        f = f.view(s*(self.g+1), c, H, W)

        f = self.base_model.conv2(f)
        f = F.avg_pool2d(f, kernel_size=f.size(2), stride=1) # GAP
       
        f = f.view(s,(self.g+1)*self.num_fltrs)
        f = self.fc1(f)
        f = F.relu(f, inplace=True)
        f = self.fc2(f)
        return f, glimpses, m, m_raw


class DenseNet_RACNN_GL(nn.Module):
    def __init__(self, num_classes=200, glimpses=2, base_pretrained=True,
             num_channels=3, use_gpu=True,
             glimpse_only=False, glimpse_size=0, skip_con=False, conv2_only=False):
        """
        glimpse_only: return glimpses of dim (s, g, x.shape)
        base_pretrained: load Imagenet weights for CNNs
        glimpses: number of glimpses
    ...
    def forward(self, x):
        ...
        f = self.fc2(f)
        return f, f_gl, m, m_raw # (s, 200), (s*g, 200), (s, g, 4), (s, g, 4)
