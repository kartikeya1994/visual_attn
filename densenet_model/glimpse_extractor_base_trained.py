...
from densenet import densenet121_pretrain
...
class DenseNet_RACNN_Glimpse_Extractor(nn.Module):
    def __init__(self, num_classes=200, glimpses=2, base_pretrained=True,
        ...
        self.conv1 = densenet121_pretrain(weights='./saved/den_cub_19_0.7875.pth')
        ...
        4 (4, 1024, 9, 9)
        """
        self.skip_concat_dim = 18**2
        if skip_con:
            self.conv1.features.denseblock3.register_forward_hook(store_activations)
            #for i in range(4):
            #    attr = getattr(self.conv1.features, 'denseblock{}'.format(i+1))
            #    attr.register_forward_hook(store_activations)
            self.glimpse_fc_skip = nn.Linear(1024 * self.skip_concat_dim,
                    self.skip_concat_dim)
            self.glimpse_fc = nn.Linear(self.skip_concat_dim, self.glimpse_dim * self.g)
        else:
    ...
    def forward(self, x):
        ...
        if self.skip_con:
            f = stored_denseblock_activations[0]
            stored_denseblock_activations = []
            f = f.view(s, 1024 * self.skip_concat_dim, -1) 
