...
from utils import BoxCarAuto, Upsampler
...
def densenet121_racnn_glimpse_extractor(weights=None, num_classes=200, 
        freeze_conv1=False, skip_con=False, use_gpu=True, glimpse_size=0):
...
class DenseNet_RACNN_Glimpse_Extractor(nn.Module):
    def __init__(self, num_classes=200, glimpses=2, base_pretrained=True,
            num_channels=3, conv_dim = 9, skip_con=False, use_gpu=True,
            glimpse_size=0):
        """
	...
	"""
        ...
        # self.batchnorm = nn.BatchNorm1d(self.glimpse_dim * self.g)

        self.cropper = BoxCarAuto(use_gpu=use_gpu)
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(f, kernel_size=f.size(2), stride=1)
        out = self.conv1.classifier(out.view(out.size(0), -1))
