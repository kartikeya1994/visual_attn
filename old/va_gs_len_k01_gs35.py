    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'validation']:
            ...
            for data in tqdm(dataloders[phase]):
                ...
                gs_max_size = 280 #(l + h)
                ml_plus_mh = m_raw[:,:,2] + m_raw[:,:,3]
                gs_size_constr = torch.sum((ml_plus_mh > gs_max_size).float() * (ml_plus_mh - gs_max_size)) 

                lamb = 0.1
                lamb2 = 0.1
                lamb3 = 0.1

                loss = l + l_gl + lamb*(lb1+lb2+ub1+ub2) + lamb2*dist_constr + lamb3 * gs_size_constr
...
model_ft = densenet121_racnn_gl(use_gpu=use_gpu, gap=False, glimpse_size=35) 
