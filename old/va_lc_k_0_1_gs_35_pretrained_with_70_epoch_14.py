    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'validation']:
            ...
            for data in tqdm(dataloders[phase]):
                ...
                ub2 = torch.sum((m_raw[:,:,2:] > H-1).float()*(m_raw[:,:,2:] + -1*H + 1))    
                
                lamb = 0.1

                loss = l + l_gl + lamb * (lb1 + lb2 + ub1 + ub2)
...
weights = './models/va_lc_k_0_14_0.7525.pth'
model_ft = densenet121_racnn_gl(weights=weights, use_gpu=use_gpu, gap=False, glimpse_size=35) 
