...
from densenet_bcauto_classify1 import densenet121_racnn_gl
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'validation']:
            ...
            for data in tqdm(dataloders[phase]):
                ...
                # forward
                p, _, m, m_raw, p_conv1 = model(inputs)
                # p is (s, 200)
                ...
                l_conv1 = criterion(p_conv1, labels)
                ...
                #max_crop_size = 140
                #ub3 = torch.sum((m_raw[:,:,2:] > max_crop_size).float()*(m_raw[:,:,2:] + -1*max_crop_size))
                ...
                loss = l + l_conv1 + lamb1*(lb1+lb2+ub1+ub2) + lamb2*dist_constr
                
                l1 += l.data[0]
                l2 += (lamb1*(lb1+lb2+ub1+ub2)).data[0]
                l3 += (lamb2*dist_constr).data[0]
                l4 += l_conv1.data[0]
            ...
            print('L1: {} L_Bound: {} L_Dist: {} L_CONV1: {}'.format(l1/float(dataset_sizes[phase]),
...
model_ft = densenet121_racnn_gl(weights='./models/va_nogl_orig_constr50_classify1_6_0.410625.pth', use_gpu=use_gpu, glimpse_size=35) 
